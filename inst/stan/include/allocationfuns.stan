  ...
  for (j in 1:n_areas) {
    for (r in 1:R) {
     ...
     for (j in 1:n_areas){
       ...
       // first push through allocation at neutral logit for local Jacobian/sensitivity
       for(r in 1:(free_R - 1)){
         for(c in 1:(free_C - 1)){
           ...
           this_inv_logit = inv_logit(neutral_logit);
       ...
       for(r in 1:(free_R - 1)){
         real sigma_sq_row = 0;
         for(k in 1:(C - 1)){
           sigma_sq_row += square(sigma_jrc[active_row_map[r], k]);
           }
         for(c in 1:(free_C - 1)){
           real sens_sq = 0;
           for(k in 1:(C - 1)){
      ...
      sens_sq += square(sens_k);
           }
           real target_sigma = sqrt(sigma_sq_row / fmax(sens_sq, 1e-10));
          ...
          ret_array[8, j, active_row_map[r], active_col_map[c]] = sigma_scale[r, c];
          ret_array[9, j, active_row_map[r], active_col_map[c]] = sigma_sq_row;
          ret_array[10, j, active_row_map[r], active_col_map[c]] = sens_sq;
       ...
       //     real log_ss = center + (width/2) * tanh((log_ss_raw - center) / (width/2));
       //     // sigma_scale[r,c] = exp(log_ss);
       //     sigma_scale[r, c] = exp(log_ss_raw);
       //     // sigma_scale[r, c] = fmin(fmax(target_sigma/fmax(tmp_J_mu[r, c], 1e-10), 0.06), 5);
       //     ret_array[8, j, r, c] = sigma_scale[r, c];
       //   }
       // }

      // second push through allocation at lambda_mu_rc for E_rc
      // reset mu vectors
       mu_slack_row = slack_row_raw[1:free_R];
       mu_slack_col = slack_col_raw[1:free_C];
       mu_rt = sum(mu_slack_row);
       ...
       for(r in 1:(free_R - 1)){
         for(c in 1:(free_C - 1)){
           ...
           local_mu_cell_value[r, c] = lower_bound + this_inv_logit * (upper_bound - lower_bound);
           tmp_J_mu[r,c] = (upper_bound - lower_bound)*this_inv_logit*(1 - this_inv_logit);
           ret_array[11, j, active_row_map[r], active_col_map[c]] = tmp_J_mu[r,c];
           mu_slack_col[c] = fmax(mu_slack_col[c] - local_mu_cell_value[r, c], 0.0);
           mu_slack_row[r] = fmax(mu_slack_row[r] - local_mu_cell_value[r, c], 0.0);
           mu_rt = fmax(mu_rt - local_mu_cell_value[r, c], 0.0);
         }
       ...
       // third push through allocation at lambda using sensitivity to make actual allocations
       for (r in 1:(free_R - 1)){
         for (c in 1:(free_C - 1)){
