...
// following code to distinguish structural zeros from sampling zeros in the log-linear model - identifying rows and columns, and number of structural zeros in the data
  ...
  int K_all = K_ame + has_area_cell_effects*(R - 1) * (C - 1);
...
transformed parameters{
  ...
  real area_row_components[n_areas, R, C];
  ...
  col_effect = rep_vector(0, C);
    ...
    if(lflag_centred_rc == 0){
      cell_effect = mu_cell_effects + scale_cell_effects*cell_effect_raw;
 ...
 if(has_L == 1){
  ...
  for(j in 1:n_areas){
    ...
    for(r in 1:R){
      for(c in 1:C){
        area_row_components[j, r, c] = area_col_effect[j, c] + area_cell_effect[j, r, c];
      }
      area_row_effect[j,r] = log(row_margins[j, r] + .00001) - log_sum_exp(to_vector(area_row_components[j, r, 1:C]));
    }

    for(r in 1:R){
        for(c in 1:C){
          log_e_cell_value[j, r, c] = area_row_effect[j, r] + area_col_effect[j, c] + area_cell_effect[j, r, c];
        }
      }
    }
...
model{
  ...
  // }
    ...
    // cell_effect_raw~ normal(0, prior_mu_ce_scale);
    // row_effect_raw~ normal(0, prior_mu_re_scale);
    // cell_effect_raw ~ normal(0, prior_cell_effect_scale);

    if(lflag_centred_c  == 0){
       col_effect_raw ~ std_normal();
    } else {
       col_effect_raw ~ normal(mu_col_effects, scale_col_effects);
    }
    if(lflag_centred_rc == 0){
       cell_effect_raw ~ std_normal();
    } else {
       cell_effect_raw ~ normal(mu_cell_effects, scale_cell_effects);
    }
