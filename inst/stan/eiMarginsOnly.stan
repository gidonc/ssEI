...
model{
  ...
  for(j in 1:n_areas){
    ...
    E_j[j] = rm_log[j, R] + cm_log[j, C] - tot_log[j] + E_j_all[j, R * C];

    for(r in 1:R - 1){
      E_jr[j, K_jr_start + r] = rm_log[j, r] - rm_log[j, R] + E_j_all[j, K_jr_start + r];
    }
    for(c in 1:C - 1){
      E_jc[j, c] = cm_log[j, c] - cm_log[j, C] + E_j_all[j, K_jc_start + c];
    }
    for(r in 1:R - 1){
      for(c in 1:C - 1){
        E_jrc[j, r, c] = E_j_all[j, K_jrc_rstart[r] + c];
      }
    }
    ...
    for(r in 1:R){
      for(c in 1:C){
        if(structural_zeros[j,r,c]==0){
          counter_cell += 1;
           e_cell[counter_cell] = exp(E_j[j] + E_jr[j, r] + E_jc[j, c] + E_jrc[j, r, c]);
  ...
  sigma_j_all ~ normal(0, prior_sigma_re_scale);
